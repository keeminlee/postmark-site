...
test("the guard, the lift, the join and the wipe each read as themselves", () => {
  ...
  // ⚑ "STEPS IN", NOT THE FERRY WORD (founder-ruled: never that word for going
  // through a door). It was printing in the live combat feed. "steps in" rather
  // than "enters" because the sentence for going the other way is already
  // "steps out" — one pair, read together in the same scrolling column.
  assert.match(join.text, /rei steps in — initiative 17, at round 3\./);
  assert.doesNotMatch(join.text, /cross/i, "the word is gone from the sentence, not merely reworded around");
...
test("stepping in is joining, and the open is the room's own event", () => {
  ...
  assert.match(out[1].text, /^rei steps in — initiative 14\./,
...
test("a new row on the wheel is somebody stepping in, and a round is a rule", () => {
  ...
  assert.match(out[1].text, /^wright steps in — initiative 14\./);
});
