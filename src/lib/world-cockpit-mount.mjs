...
import {
  ...
  // ⚑ `briefWords` was imported here and is not any more (2026-08-29). It cut the
  // door's sentence down to fit the small name box; the box is gone, so nothing
  // in this file has a width to fit a sentence into. It stays exported from
  // world-cockpit.mjs with its own tests — a pure function nobody calls today is
  // cheaper to keep than to re-derive, and the rail may yet want it.
...
/* ⚑ THE STAND-DOWN RULE STOOD HERE AND IS GONE (2026-08-29, later the same day).
   It read .pmc-roster:has(.pmc-face:hover) .pmc-here { opacity: 0 } — the plate
   hid itself while the pointer was on a face, so that the face's own small name
   box was the only card up. That was the right fix for "two overlapping cards"
   while both cards existed. The founder's next ruling settled which of the two
   survives, the other way round: the hover "should show the orange-rimmed LARGER
   card, not the small nameplate." So the small box is deleted (see drawRoster)
   and the plate is exactly what a face hover reveals — a rule that hid it on
   face hover would now hide the answer. */
...
/* ⚑ THE .says RULE STOOD HERE AND IS GONE with it. It dressed the door's whole
   sentence about why a human may stand here, moved onto the plate when the name
   box was shortened. The sentence is not orphaned — it is a clause in the plate's
   stat row now (see statRowHtml), where it sits beside the other things known
   about the fighter rather than under a rule of its own. */
...
/* ⚑ THE GLYPH'S OWN MARGIN HAD TO BE UNDONE, and only a shot could have found it.
   .pmc-ico is drawn ABOVE a verb's name in the bar, so it carries margin: 0 auto
   to centre itself over the word. In a flex ROW an auto margin eats every pixel
   of free space instead: the hand floated alone in the middle of the plate with
   the thing's name shoved against the right edge, a row that measured perfectly
   in the DOM and read as two unrelated fragments on the screen. And it is sized
   up here, because 1.15em of a .78rem row is an 11px picture — too small to say
   "held" at the size it is actually drawn. */
.pmc-here .kit .pmc-ico {
  margin: 0; flex: 0 0 auto; width: 1.45em; height: 1.45em;
  color: var(--pmc-gold); opacity: 1;
}
.pmc-here .kit.none .pmc-ico { color: var(--pmc-dim); }
...
export function mountCockpit(o) {
  ...
  function drawRoster() {
    ...
    const face = (f) => {
      ...
      // the plate from the dock — which is the doubling he was looking at. The
      // earlier fix that day hid the PLATE on a face hover; this ruling settles
      // it the other way, so that rule went with the box (see COCKPIT_CSS).
      //
      ...
      //
      // THE WHOLE SENTENCE RIDES ON `aria-label` — the box was the only place a
      // screen reader met the door's reason, and the plate is `role="tooltip"`
      // that no assistive cursor is aimed at from here. So the label is the long
      // form, not the bare handle it was while the box existed. `briefWords`
      // shortened it for a 2.3em circle's box; nothing shortens it now, because
      // nothing is being fitted into a box.
   ...
   */
  const ICON_HAND =
    `<svg class="pmc-ico" viewBox="0 0 24 24" aria-hidden="true" focusable="false"><path d="M12 3v12 M8 8h8 M9 15h6v5H9z"/></svg>`;
   ...
   *
   * ⚑ IT IS KEPT AND IT IS NARROWED, rather than going down with the recitation
   * around it. It began as the dock's `title`, moved onto the plate when that
   * tooltip became a third card on one hover, and the ask that emptied the plate
   * was aimed at the standpoint recitation ("useless for the human to see"), not
   * at a disclosure. A law about what the surface records is not the kind of
   * sentence to let fall on the floor while nobody is looking at it.
   *
   * ONLY OVER YOUR OWN SEAT. It is about YOUR hand, and the plate is now a way to
   * look at other people — printed under an ally's hit points it would read as a
   * claim about them, which is both wrong and the sort of noise this plate was
   * just cleared of.
   */
   ...
   *  Each clause is dropped where the door did not answer it, rather than
   *  printed with a dash — a plate of em-dashes is the debug panel again. */
  function statRowHtml(s, f) {
    const bits = [];
    ...
    // the human's own row carries the door's sentence about why a human may
    // stand here at all — the one thing `humanWords` was reading for
    if (f?.kind === "human" && f.allowed) bits.push(humanWords(f));
